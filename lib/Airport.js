...
Airport.Display = function(obj) {
 ...
 // evtl. überflüssig
 this.loop = function() {
     var s = "AirportDisplays[" + self.id + "].loop()"; 
 ...
 this.animate = function() {
   ...
   if (self.actual < self.phrases.length)
    ...
    s = "AirportDisplays[" + self.id + "].animate()"; 
    window.setTimeout(s, obj.timeout);  
    }
  else { 
       self.actual = 0; 
 ...
 this.init = function(){
   self.id = AirportDisplays.length;
   AirportDisplays.push(this);  
   
   self.create_phrases();
   // self.phrases[0].shuffle();
   // self.phrases[1].shuffle();   
   ...
   AirportDisplays[self.id].animate();     
   };
